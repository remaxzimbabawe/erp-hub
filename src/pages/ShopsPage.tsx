        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea placeholder="Describe this shop location..." {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
...
export default function ShopsPage() {
  ...
  const handleUpdate = (data: ShopFormData) => {
    ...
    refreshData();
    setEditItem(null);
    toast({ title: "Shop updated", description: `${data.name} has been updated.` });
  };

  const handleDelete = () => {
    if (!deleteItem) return;
    deleteShop(deleteItem._id);
    refreshData();
    setDeleteItem(null);
    toast({ title: "Shop deleted", description: "The shop has been removed." });
  };

  const handleView = (shop: Shop) => {
    navigate(`/pos?shop=${shop._id}`);
  };
  ...
  const columns = [
    ...
    { key: "description" as const, header: "Description" },
    {
      ...
      header: "Products",
      render: (item: Shop) => getProductsByShop(item._id).length,
    },
    {
      key: "clients" as const,
      header: "Clients",
      render: (item: Shop) => getClientsByShop(item._id).length,
    },
  ];
