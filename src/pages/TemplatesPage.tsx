...
function TemplateForm({
  ...
  defaultValues: ProductTemplateFormData;
  categories: ProductCategory[];
  onSubmit: (data: ProductTemplateFormData) => void;
  submitLabel: string;
}) {
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="e.g., Cola 500ml" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="e.g., COLA500" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
...
export default function TemplatesPage() {
  ...
  const handleUpdate = (data: ProductTemplateFormData) => {
    if (!editItem) return;
    updateProductTemplate(editItem._id, {
      name: data.name,
      code: data.code,
      productCategoryId: data.productCategoryId,
      priceInCents: data.priceInCents,
  ...
  const handleDelete = () => {
    ...
    deleteProductTemplate(deleteItem._id);
    refreshData();
    setDeleteItem(null);
  ...
  const columns = [
    ...
    {
      key: "productCategoryId" as const,
      header: "Category",
      render: (item: ProductTemplate) => (
        <Badge variant="secondary">{getCategoryName(item.productCategoryId)}</Badge>
      ),
    },
    {
      key: "priceInCents" as const,
      header: "Price",
      render: (item: ProductTemplate) => (
        <span className="font-semibold">{formatPrice(item.priceInCents)}</span>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        ...
        description="Manage product templates that can be added to shops"
      >
      ...
      >
        <TemplateForm
          defaultValues={{ name: "", code: "", productCategoryId: "", priceInCents: 0 }}
      ...
      {editItem && (
        <ModalForm
          ...
          title="Edit Template"
          description="Update product template details"
        >
          <TemplateForm
            key={editItem._id}
            defaultValues={{
              name: editItem.name,
              code: editItem.code || "",
              productCategoryId: editItem.productCategoryId,
              priceInCents: editItem.priceInCents,
            }}
            categories={categories}
